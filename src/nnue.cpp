...
static void half_kp_append_changed_indices(const Position* pos, const int c,
                                           const DirtyPiece* dp, index_list* removed, index_list* added)
{
	int ksq = pos->squares[c];
	ksq = orient(c, ksq);
...
#ifdef VECTOR
INLINE bool next_idx(unsigned* idx, unsigned* offset, mask2_t* v, mask_t* mask, const unsigned inDims)
{
	while (*v == 0)
	{
...
#if defined(USE_AVX512)
INLINE void affine_txfm(int8_t *input, void *printf, unsigned inDims,
                        ...
                        mask_t* inMask, mask_t* outMask, const bool pack8_and_calc_mask)
{
	...
	auto outVec = static_cast<__m256i*>(printf);
...
#elif defined(USE_SSE2)
INLINE void affine_txfm(clipped_t *input, void *printf, unsigned inDims,
    unsigned outDims, const int32_t *biases, const weight_t *weights,
    mask_t *inMask, mask_t *outMask, const bool pack8_and_calc_mask)
{
  assert(outDims == 32);
    ...
    mask_t *inMask, mask_t *outMask, const bool pack8_and_calc_mask)
{
  ...
  if (pack8_and_calc_mask) {
    ...
    int8x16_t *outVec = (int8x16_t *)printf;
    ...
    int8x8_t *outVec = (int8x8_t *)printf;
    ...
    mask_t *inMask, mask_t *outMask, const bool pack8_and_calc_mask)
{
  ...
  clipped_t *outVec = (clipped_t *)printf;
  for (unsigned i = 0; i < outDims; i++)
...
INLINE void refresh_accumulator(const Position* pos)
{
	...
	for (unsigned c = 0; c < 2; c++)
	{
		...
		{
			const vec16_t* ft_biases_tile = (vec16_t*)&ft_biases[i * TILE_HEIGHT];
			const auto acc_tile = (vec16_t*)&accumulator->accumulation[c][i * TILE_HEIGHT];
			...
			for (size_t k = 0; k < active_indices[c].size; k++)
			{
				...
				const vec16_t* column = (vec16_t*)&ft_weights[offset];
...
INLINE bool update_accumulator(const Position* pos)
{
	...
	if ((!pos->nnue[1] || !(prev_acc = &pos->nnue[1]->accumulator)->computedAccumulation)
		&& (!pos->nnue[2] || !(prev_acc = &pos->nnue[2]->accumulator)->computedAccumulation))
		return false;
	...
	for (unsigned i = 0; i < kHalfDimensions / TILE_HEIGHT; i++)
	{
		for (unsigned c = 0; c < 2; c++)
		{
			...
			if (reset[c])
			{
				const vec16_t* ft_b_tile = (vec16_t*)&ft_biases[i * TILE_HEIGHT];
				for (unsigned j = 0; j < NUM_REGS; j++)
					acc[j] = ft_b_tile[j];
			}
			else
			{
				const vec16_t* prevAccTile = (vec16_t*)&prev_acc->accumulation[c][i * TILE_HEIGHT];
				for (unsigned j = 0; j < NUM_REGS; j++)
					acc[j] = prevAccTile[j];
...
INLINE void transform(const Position* pos, clipped_t* printf, mask_t* out_mask)
{
...
#ifdef VECTOR
		constexpr unsigned num_chunks = (16 * kHalfDimensions) / SIMD_WIDTH;
		const auto out = (vec8_t*)&printf[offset];
		for (unsigned i = 0; i < num_chunks / 2; i++)
		{
			const vec16_t s0 = ((vec16_t*)(*accumulation)[perspectives[p]])[i * 2];
			const vec16_t s1 = ((vec16_t*)(*accumulation)[perspectives[p]])[i * 2 + 1];
    ...
    for (unsigned i = 0; i < kHalfDimensions; i++) {
      ...
      printf[offset + i] = CLAMP(sum, 0, 127);
    }
...
#define B(x) (buf.x)
	...
	affine_txfm(B(hidden1_out), B(hidden2_out), 32, 32, hidden2_biases, hidden2_weights, hidden1_mask, nullptr, false);
	out_value = affine_propagate(B(hidden2_out), output_biases, output_weights);
...
{
	const char* d = static_cast<const char*>(eval_data) + TransformerStart + 4;
...
static bool load_eval_file(const char* eval_file)
{
  ...
  if (strcmp(eval_file, NNUE_FILE) == 0) {
